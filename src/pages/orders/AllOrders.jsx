...
const AllOrders = () => {
  ...
  const handleCustomerSelect = (customerId) => {
    ...
    if (customer) {
      ...
      const addressString = defaultAddress ? formatAddress(defaultAddress) : '';
  ...
  const handleAddressSelect = (addressId) => {
    ...
    if (address) {
      manualOrderForm.setFieldsValue({ deliveryAddress: formatAddress(address) });
    }
  };
  ...
  const handleAddNewAddress = async (values) => {
    ...
    try {
      ...
      `, { ...values, landmark: values.landmark || null, isDefault: values.isDefault || false });
  ...
  const handleAddItem = () => {
    const newItem = { id: Date.now().toString(), productId: '', quantity: 1, price: 0, total: 0 };
  ...
  const handleAddCustomerSuccess = (newCustomer) => {
    ...
    fetchCustomersForOrder('');
    message.success('Customer added successfully! You can now select them from the dropdown.');
  };
  ...
  const handleManualOrderSubmit = async (values) => {
    ...
    try {
      ...
      const result = await createAdminOrder(parseInt(selectedCustomer.id), values.deliveryAddress, itemsForMutation);
      if (result.success && result.order) {
        message.success(`Order ${result.order.orderNumber} created successfully!`);
        manualOrderForm.resetFields();
        setSelectedCustomer(null);
        setOrderItems([]);
  ...
  return (
    <div>
      ...
      <Card
        ...
        extra={
          <Space>
            <Button type="primary" icon={<PlusOutlined />} onClick={handleOpenManualOrder}>
              Take Order
            </Button>
                ...
                return (
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {orderStatuses.map((status, index) => {
                      ...
                      // Get tracking data for this status if available
